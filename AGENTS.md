...
- 采用「锚点定位法」：从 `next` 头部取短锚点（默认 `16` 字，多个起始偏移以跳过顶部截断噪声），在 `previous` 尾部滑动定位对齐位置，据此推算任意长度的重叠——重叠长度不再受固定上限限制（旧的等长后缀/前缀法在重叠超过上限时会误判断层，尤其是末屏滚动被 clamp 的场景）。
- 默认最小重叠长度 `10`，相似度阈值 `0.85`。搜索回溯上限 `maxOverlap` 仅作为超长文本的安全护栏；真实重叠不会超过 `next` 长度，由其自然限定。
- 候选校验顺序为「锚点相似度优先，其次越靠近 `previous` 尾部越优先」，并仅比对前 `verifyLength`（默认 `64`）字以区分真接缝与偶然短串，避免对超长重叠做整段 Levenshtein；最多校验 `maxVerifications`（默认 `16`）个候选。
- 必须优先选择相似度最高的结果；不能直接选择最长可接受候选，否则容易过度截断。
- 找不到可靠接缝时必须保留两段文本，并插入 `[可能存在断层，需人工校对]`。
- 不允许为了追求连贯而静默丢弃无法确认的文本。
- OCR 常把中文句读识别成半角 `, ; : ! ?`，`normalizeOcrText` 会在中文语境（两侧紧邻中文，允许隔空格）下转回全角并清理空格；数字千分位、英文等非中文语境保持原样。
